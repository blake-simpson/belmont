...
**Why this matters.** `(depends: M<n>)` on a milestone heading used to have one implementation and one caller: `computeWaves`, from `runAutoParallel`. Everything else — `nextMilestone`, `nextTask`, both status views, `/belmont:implement`'s prose selection, `/belmont:next`'s, `/belmont:loop`'s step 0 — was document-order only. So on a feature whose first pending milestone declared unmet deps (`### M16: … (depends: M15, M21, M22)` with M21/M22 not started), every surface that recommends work named the one milestone that must run last, and `belmont auto`'s interactive range prompt defaulted to a range "starting" at it (issue #59, live on PR #32's feature). Single-feature execution was never wrong — `autocmd.go` routes to wave-ordered `runAutoParallel` whenever any in-range milestone has deps, so `computeWaves` reorders whatever it is given. On that path the damage was entirely to what humans and skills were told. Multi-feature mode is another matter: `--features`/`--all` run each feature's whole milestone set through `runLoop` (`runFeatureInWorktree`, auto_parallel.go:90) with **no** milestone-level wave ordering and a dep-blind decision engine — a pre-existing gap this change does not close, tracked as #62.
...
- **An empty milestone never satisfies a dependency.** `milestoneAllDone` requires at least one task, so a dep on a zero-task milestone is permanently unmet — on both paths: the prose selectors say "at least one task" explicitly, matching the predicate. This is `computeWaves` parity, not an accident, and the by-product is real: a dep on an accidentally-emptied milestone renders "(waiting on dependencies)" forever, the empty milestone itself is offered as Next (it is not-all-done), and nothing lints the shape — a planning error `/belmont:tech-plan` owns.
...
- **"Add deps to the AI-decision `milestone_states` JSON."** Deferred — and the first version of this bullet overclaimed why, so be precise: `runLoop` has three callers. The single-feature path reaches it only dep-free (`autocmd.go` routes to `runAutoParallel` otherwise), and the per-milestone worktree path scopes the range to one milestone whose deps its wave already satisfied — but the multi-feature path (`runFeatureInWorktree`, auto_parallel.go:90) reaches the decision engine with dep-carrying milestones in range and nothing consulting them. Adding the JSON field only matters once that path's *execution* is dep-aware at all, which is #62's whole fix, so it belongs there — most likely as `decideLoopAction*` consulting `depSatisfied` rather than as advice to the AI.
...
- 2026-08-18 — Red-team round on the same PR: corrected the `runLoop`-reachability claim (multi-feature mode reaches the decision engine dep-blind — pre-existing, filed #62) and scoped "execution was never wrong" to the single-feature path; recorded the empty-milestone ruling; `milestoneStatusWord` gained the blocked branch `milestoneStatusIcon` always had, and the task line gained `nextTaskBlockedByDeps` so it never reads a bare "None" while a workable task hides behind a dependency.
