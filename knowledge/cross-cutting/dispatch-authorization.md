...
- 2026-08-18 — red-team round on the same PR: the trio's gap split out as #63 and referenced from Known rough edges, and the agent-teams section's "for the four that include this partial" corrected — it sat one section below the same entry's new "five skills" count. (This line itself repairs a process miss: the previous edit changed the entry without recording it here.)
